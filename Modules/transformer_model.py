...
class CTCVAE(nn.Module):
    ...
    def forward(self, x, target, loss_mask):
        ...
        input_encoder, y_type = self.encoder(x, is_training=True)
    ...
    # 计算出  隐向量
    def encoder(self, x, memory=None, is_training=True):
        ...
        # transformer
        if is_training:
            # mask 邻接矩阵  句子中词是否mask   加了Attention  1024*1024矩阵做注意力
    ...
    def forward_output(self, input_encoder, target):
        '''
        for training
        '''
        # tf_skip_emption = self.word_emb_emotion(y[..., 7])
        tf_skip_type = self.word_emb_type(target[..., 3])
    ...
    def generate_from_scratch(self, dictionary, emotion_tag, key_tag=None, n_token=8, display=True):
        ...
        with torch.no_grad():
            ...
            for step in range(init.shape[0]):
                ...
                # 采样类型
                h, y_type, memory = self.encoder(input_, memory, is_training=False)
